...
from views.game_view import create_game_window, draw_grid, draw_canon
...
def count_point(config):
    compte = 0
    for x in range(config.n + 1):
        for y in range(config.n + 1):
            if config.grille[x][y] == config.joueur_actuel:
                compte += 1
    return compte
...
def manage_click(event, config):
    ...
    if 0 <= hx <= config.n and 0 <= hy <= config.n and config.grille[hx][hy] == 0:
        ...
        win_found = False
        seuil = 4 if count_point(config) >= 10 else 5
        ...
        if win_found:
            config.label_score_j1.config(text=f"{config.j1} : {config.score_j1}")
            config.label_score_j2.config(text=f"{config.j2} : {config.score_j2}")
...
def restaurer_visuel_grille(config):
    ...
    for (x1, y1, x2, y2) in config.alignements:
        config.canvas.create_line(x1*pas, y1*pas, x2*pas, y2*pas, fill="#00ccff", width=5)
...
def charger_partie_selectionnee(donnees_mongo, root, config_actuelle):
    ...
    config_actuelle.alignements = donnees_mongo.get('alignements', [])
